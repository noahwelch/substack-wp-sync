...
### Rollback & Management
- **Flexible Rollback:** Remove all posts, failed posts only, or posts within date ranges
- **Safe Deletion:** Confirmation dialogs prevent accidental data loss
- **Database Cleanup:** Removes both WordPress posts and sync log entries

## Database Schema

The plugin creates a custom table `wp_substack_sync_log`:
